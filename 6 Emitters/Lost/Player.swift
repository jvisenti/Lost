...
final class Player : SKNode {
    ...
    var movementSpeed = CGFloat(0.0) {
        didSet {
           emitter.particleBirthRate = (movementSpeed > 0.0) ? 200.0 : 0.0
        }
    }
    ...
    private let emitter = SKEmitterNode(fileNamed: "rocket")!
    ...
    override init () {
        ...
        emitter.position = CGPoint(x: -0.3 * background.size.width, y: 0.0)
        emitter.particleBirthRate = 0.0
        emitter.particleZPosition = 1.0
    ...
    func update(dt: NSTimeInterval) {
        ...
        // Particles will leave a trail behind the player
        emitter.targetNode = parent
        emitter.emissionAngle = zRotation + CGFloat(M_PI)
    }
